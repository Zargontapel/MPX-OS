...
void insertMCB(MemoryBlockList* list, CompleteMCB* block) {
  if (list->head == NULL) {
    ...
    CompleteMCB* pointer = list->head;
	CompleteMCB* oldPointer = pointer;
    while (pointer->beginningAddress < block->beginningAddress && pointer != NULL)
	{
		oldPointer = pointer;
		pointer = pointer->next;
	}
...
void print_alloc(void)
{
	...
	while(currMCB != NULL)
	{
		...
		serial_print("    Offset: ");
		serial_println(itoa(currMCB->beginningAddress + sizeof(CompleteMCB) - heapBeginning));
		serial_print("    Size: ");
