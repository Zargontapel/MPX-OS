...
void loadBasicFunctionToPCB(char name[10], unsigned char class, unsigned char priority, unsigned char readyState, unsigned char suspendedState, int stackSize, void (*func)())
{
...
void* allocateMem()
{
	...
	serial_println(itoa(ptr - heapBeginning + sizeof(CompleteMCB)));
	if(ptr != NULL)
	{
		return (void *) ptr;
	}
