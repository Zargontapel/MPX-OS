...
void kmain(void)
{
   ...
   klogv("Initializing virtual memory...");
	sys_set_malloc(my_alloc);
	sys_set_free(my_free);
   // 5) Call Commhand
   klogv("Transferring control to commhand...");
   init_heap(40000);
   void (*commandhandling)() = &comhandle;
   loadBasicFunctionToPCB("Commhand", 1, 5, 0, 0, 2048, commandhandling);
   asm volatile ("int $60");
   void (*idlefunc)() = &idle;
   loadBasicFunctionToPCB("Idle", 1, 0, 0, 0, 128, idlefunc);
   asm volatile ("int $60");
