...
void version()
{
	serial_println("Version 1.0 Module R3");
	serial_println("Completion Date: Wednesday, March 18, 2015. Sometime in the middle of the night.");
	serial_println("");
}
...
void help()
{
	while(1) {
		...
		if (!strcmpigncase(command, "commands")) {
		  ...
		  serial_println("Causes the command handler to yield CPU control to allow ready process to execute.");
		  break;
		}

		else if (!strcmpigncase(command, "loadr3")) {
		  serial_println("Loads all r3 proccess into memory in a ready blocked state. Use 'yield' command to begin their execution.");
		  break;
		}
...
{
	void (*func1)() = &proc1;
	loadBasicFunctionToPCB("proc1", 0, 1, 0, 1, 512, func1);
	void (*func2)() = &proc2;
	loadBasicFunctionToPCB("proc2", 0, 1, 0, 1, 512, func2);
	void (*func3)() = &proc3;
	loadBasicFunctionToPCB("proc3", 0, 1, 0, 1, 512, func3);
	void (*func4)() = &proc4;
	loadBasicFunctionToPCB("proc4", 0, 1, 0, 1, 512, func4);
	void (*func5)() = &proc5;
	loadBasicFunctionToPCB("proc5", 0, 1, 0, 1, 512, func5);
}
