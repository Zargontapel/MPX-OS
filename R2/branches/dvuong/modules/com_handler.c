...
void comhandle()
{
	...
	while(1) {
		...
		if (!strcmpigncase(command, "shutdown")) {
			...
			resumePCB();
		} else if (!strcmpigncase(command, "priority")) {
			setPriority();
...
void suspendPCB()
{
	...
	if(target == NULL)
	{
		serial_println("Process not found: invalid name");
	}
	else
	{
		findPCB(name)->suspendedState = 1;
	}
}
...
void version()
{
	serial_println("Version 2.0 Module R2");
