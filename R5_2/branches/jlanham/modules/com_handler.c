...
void help()
{
	while(1) {
		...
		if (!strcmpigncase(command, "commands")) {
			...
			serial_println("States whether or not the heap is entirely empty.");
		}
		else if (!strcmpigncase(command, "printfree")) {
...
void* allocateMem()
{
	...
	serial_println(itoa(ptr - heapBeginning));
...
void freeMem()
{
	...
	char* addr = polling();
	u32int address = atoi(addr);
	int result = my_free((void *)address);
	if(result == 1)
	{
